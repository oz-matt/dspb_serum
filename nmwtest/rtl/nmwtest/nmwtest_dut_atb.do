...
if [info exists ::env(QUARTUS_ROOTDIR_OVERRIDE)] {
    ...
    quietly set quartus_dir "D:/intelFPGA/16.1"
}
...
if {! [file exists $base_dir/nmwtest_dut]} {
    file delete -force $base_dir/nmwtest_dut
}

project new $base_dir nmwtest_dut
...
if {[info exists quartus_dir] && [file isdirectory $quartus_dir]} {
    ...
    do "$base_dir/nmwtest/nmwtest_dut_fpc.do"
} else {
...
vcom -quiet $base_dir/nmwtest/nmwtest_dut_safe_path_msim.vhd
vcom -quiet $base_dir/nmwtest/nmwtest_dut.vhd
vcom -quiet $base_dir/nmwtest/nmwtest_dut_filter1.vhd
vcom -quiet $base_dir/nmwtest/nmwtest_dut_scale1.vhd
vcom -quiet -O0 $base_dir/nmwtest/nmwtest_dut_atb.vhd
vcom -quiet $base_dir/nmwtest/nmwtest_dut_stm.vhd
...
if {$vcomfailed == 0} {
    onbreak {
        ...
        if {$regOK == 1} {
            ...
            quietly set cyclenum [expr int($simtime / 16000.000000)];
            if { [catch {exa mismatch_dout_0} mismatch] == 0 && $mismatch } {
                report_mismatch dout_0 $cyclenum
            }
            if { [catch {exa mismatch_dout_1} mismatch] == 0 && $mismatch } {
                report_mismatch dout_1 $cyclenum
            }
            if { [catch {exa mismatch_dout_2} mismatch] == 0 && $mismatch } {
                report_mismatch dout_2 $cyclenum
            }
            if { [catch {exa mismatch_dout_3} mismatch] == 0 && $mismatch } {
                report_mismatch dout_3 $cyclenum
            }
    ...
    vsim -quiet -error 3473 -msgmode both -voptargs="+acc" -t ps nmwtest_dut_atb 
    do $base_dir/nmwtest/nmwtest_dut_atb.wav.do
